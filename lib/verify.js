...
module.exports = async (data, environment) => {
  ...
  const repo = manifest.metadata.annotations['github.com/repo'] || null
  const sha = manifest.metadata.annotations['github.com/sha'] || null

  if (!repo) {
    throw new Error('github.com/repo annotation is missing from the manifest')
  }

  if (!sha) {
    throw new Error('github.com/sha annotation is missing from the manifest')
  }
